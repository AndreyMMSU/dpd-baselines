...
def main():
    ...
    model.train()
    for epoch in range(1, n_train - 3):
        xb = x_t[3:][epoch : epoch + 1].to(device)  
        yb = y_t[3:][epoch : epoch + 1].to(device)
        ...
        print(f"epoch {epoch:02d} | mse={loss:.3f} | NMSE={nmse_db_step:.3f}")
